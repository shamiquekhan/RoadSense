...
def run_checks(gdf: gpd.GeoDataFrame, df_vals: pd.DataFrame = None) -> pd.DataFrame:
    ...
    try:
        import joblib
        scalers_path = Path(gdf.source) if hasattr(gdf, 'source') and gdf.source else None
    except Exception:
        scalers_path = None
    ...
    if 'SpeedLimit' in df_vals.columns:
        checks['speedlimit_valid'] = df_vals['SpeedLimit'].apply(lambda v: pd.notna(v) and (v > 0) and (v < 200))
    else:
        checks['speedlimit_valid'] = pd.Series([False] * len(df))

    # MedianSpeed > 0
    if 'MedianSpeed' in df_vals.columns:
        checks['median_positive'] = df_vals['MedianSpeed'].apply(lambda v: pd.notna(v) and (v > 0))
    else:
    ...
    if 'F85thPercentileSpeed' in df_vals.columns and 'MedianSpeed' in df_vals.columns:
        checks['p85_ge_median'] = (
            df_vals['F85thPercentileSpeed'].fillna(-np.inf) >= df_vals['MedianSpeed'].fillna(np.inf)
        )
    else:
    ...
    if 'PercentOverLimit' in df_vals.columns:
        checks['percent_overlimit_ok'] = df_vals['PercentOverLimit'].apply(lambda v: pd.notna(v) and (v >= 0) and (v <= 1))
    else:
        checks['percent_overlimit_ok'] = pd.Series([False] * len(df))

    # WeightedSample > 0
    if 'WeightedSample' in df_vals.columns:
        checks['weighted_sample_positive'] = df_vals['WeightedSample'].apply(lambda v: pd.notna(v) and (v > 0))
    else:
...
def summarize_checks(checks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in checks_df.columns:
        ...
        summary.append({'check': col, 'ok_count': n_ok, 'ok_frac': n_ok / total if total else 0, 'total': total})
    return pd.DataFrame(summary)


def write_report(report_path: Path, summary_df: pd.DataFrame, checks_df: pd.DataFrame, gdf: gpd.GeoDataFrame, flagged_csv: Path):
    ...
    if not flagged.empty:
        ...
        flagged_copy['geometry'] = flagged_copy.geometry.apply(lambda g: g.wkt if g is not None else '')
    ...
    # build HTML report
    pieces = ["<html><head><meta charset='utf-8'><title>QA Summary</title></head><body>"]
    ...
    if not flagged.empty:
        ...
        sample_html['geometry'] = sample_html.geometry.apply(lambda g: g.wkt if g is not None else '')
...
def main():
    p = argparse.ArgumentParser(description='Run data quality checks on processed road safety data')
    ...
    try:
        ...
        if std_path.exists() or mm_path.exists():
            ...
            df_vals = gdf.copy()
            # standard cols
            standard_cols = [c for c in ["MedianSpeed", "F85thPercentileSpeed", "speed_excess_85", "speed_excess_median", "log_weighted_sample", "log_sample_size"] if c in df_vals.columns]
            minmax_cols = [c for c in ["SpeedLimit", "Shape_Length"] if c in df_vals.columns]
            if std_path.exists() and standard_cols:
                ss = joblib.load(std_path)
                try:
                    df_vals[standard_cols] = ss.inverse_transform(df_vals[standard_cols])
                except Exception:
                    pass
